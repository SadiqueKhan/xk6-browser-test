...
func (h *ElementHandle) IsChecked() bool {
	...
	if err != nil && err != ErrTimedOut { // We don't care anout timeout errors here!
		k6ext.Panic(h.ctx, "getting element is checked: %w", err)
	}
	return result
}
...
func (h *ElementHandle) IsDisabled() bool {
	...
	if err != nil && err != ErrTimedOut { // We don't care anout timeout errors here!
		k6ext.Panic(h.ctx, "getting element is disabled: %w", err)
	}
	return result
}
...
func (h *ElementHandle) IsEditable() bool {
	...
	if err != nil && err != ErrTimedOut { // We don't care anout timeout errors here!
		k6ext.Panic(h.ctx, "getting element is editable: %w", err)
	}
	return result
}
...
func (h *ElementHandle) IsEnabled() bool {
	...
	if err != nil && err != ErrTimedOut { // We don't care anout timeout errors here!
		k6ext.Panic(h.ctx, "getting element is enabled: %w", err)
	}
	return result
}
...
func (h *ElementHandle) IsHidden() bool {
	...
	if err != nil && err != ErrTimedOut { // We don't care anout timeout errors here!
		k6ext.Panic(h.ctx, "getting element is hidden: %w", err)
	}
	return result
}
...
func (h *ElementHandle) IsVisible() bool {
	...
	if err != nil && err != ErrTimedOut { // We don't care anout timeout errors here!
		k6ext.Panic(h.ctx, "getting element is visible: %w", err)
	}
	return result
}
