	...
	case "background_page":
		...
		if err != nil {
			...
			case <-b.ctx.Done():
				...
				return // ignore
			default:
				k6ext.Panic(b.ctx, "creating NewPage for background_page event: %w", err)
			}
		}
	...
	case "page":
		...
		if err != nil {
			...
			case <-b.ctx.Done():
				...
				return // ignore
			default:
				k6ext.Panic(b.ctx, "creating NewPage for page event: %w", err)
			}
		}
...
func (b *Browser) newPageInContext(id cdp.BrowserContextID) (*Page, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("creating a blank page in browser context %q: %w", id, err)
	}
...
func (b *Browser) Close() {
	defer func() {
		if err := b.browserProc.userDataDir.Cleanup(); err != nil {
			b.logger.Errorf("closing the browser", "%v", err)
		}
	}()
...
func (b *Browser) NewContext(opts goja.Value) api.BrowserContext {
	...
	if err != nil {
		k6ext.Panic(b.ctx, "creating browser context %q: %w", browserContextID, err)
	}
